...
param(
    ...
    [string]$AgentDownloadUrl = "https://autopilotmonitor.blob.core.windows.net/agent/AutopilotMonitor-Agent-dev.zip",
...
try {
    ...
    if (Test-Path $agentExePath) {
        ...
        try {
            ...
            $versionJsonUrl = $AgentDownloadUrl -replace '[^/]+$', 'version-dev.json'
            $expectedSha256 = $null
            ...
            $zipPath = Join-Path $env:TEMP "AutopilotMonitor-Agent-dev.zip"
