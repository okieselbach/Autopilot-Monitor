# Tenant Config Report (Global Admin)

## Goal
New global-admin-only page that shows a read-only report of a tenant's full configuration,
including admin-set parameters and computed runtime (agent) parameters.

## Plan
- [x] Write plan
- [ ] Create `/app/tenant-config-report/page.tsx` — global admin only, read-only report
...
- [ ] Add nav item to global admin section in `globalNavConfig.tsx`
