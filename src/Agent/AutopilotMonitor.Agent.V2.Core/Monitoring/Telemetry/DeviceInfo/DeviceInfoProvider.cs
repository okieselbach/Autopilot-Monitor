...
namespace AutopilotMonitor.Agent.V2.Core.Monitoring.Telemetry.DeviceInfo
{
    ...
    public static class DeviceInfoProvider
    {
        ...
        public static bool IsVirtualMachine(string manufacturer, string model)
        {
            ...
            // Unambiguous VM manufacturers — none of these vendors ship physical hardware
            // where Win32_ComputerSystem.Manufacturer carries their name on the host.
            ...
            // "Microsoft Corporation" is shared between Hyper-V VMs (incl. Cloud PC, AVD)
            // and physical Surface devices. Hyper-V reports Model="Virtual Machine"; Surface
            // reports e.g. "Surface Laptop 5".
            if (mdl.IndexOf("Virtual Machine", StringComparison.OrdinalIgnoreCase) >= 0)
                return true;

            // Defensive: some hypervisors set the model string instead of manufacturer.
