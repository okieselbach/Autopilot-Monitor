...
namespace AutopilotMonitor.Agent.V2.Core.Orchestration
{
    ...
    public sealed class EnrollmentOrchestrator : IQuarantineSink, IDisposable
    {
        ...
        // TRACE-H2: a prior run that could no longer persist consistently leaves a marker
        // (see RecoveryCoordinator.QuarantineRequestedMarkerFile) that the NEXT start honours.
        // These flags capture whether that happened this start, for the step 13b-Q event.
        ...
        public void Start(Action<ISignalIngressSink>? onIngressReady = null)
        {
            ...
            // 1+2) Crash-recovery state machine (Plan §2.7). Extracted to RecoveryCoordinator
            //       (ARCH-F5): it honours a prior-run quarantine marker, performs the WhiteGlove
            //       Part-2 archive-and-reset, opens the persistence writers against the resulting
            //       clean directory, picks the recovery branch + replay stream, aligns the journal
            //       AHEAD-phantom boundary, and folds to the seed state. The writers + flags it
            //       returns drive the wiring steps below; field names (_signalLog, _journal, …)
            //       are preserved for the recovery tests that reflect on them.
            var recovery = RecoveryCoordinator.Recover(_stateDirectory, _sessionId, _tenantId, _clock, _logger);
            _signalLog = recovery.SignalLog;
            _journal = recovery.Journal;
            _snapshot = recovery.Snapshot;
            _eventSequencePersistence = recovery.EventSequence;
            _isWhiteGlovePart2 = recovery.IsWhiteGlovePart2;
            _wasStartupQuarantine = recovery.WasStartupQuarantine;
            _priorRunQuarantined = recovery.PriorRunQuarantined;
            _priorRunQuarantineReason = recovery.PriorRunQuarantineReason;
            var initialState = recovery.InitialState;
        ...
        public void TriggerQuarantine(string reason)
        {
            ...
            try
            {
                var markerPath = Path.Combine(_stateDirectory, RecoveryCoordinator.QuarantineRequestedMarkerFile);
