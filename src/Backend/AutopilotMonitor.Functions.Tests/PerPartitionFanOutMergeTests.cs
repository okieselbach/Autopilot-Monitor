...
public class PerPartitionFanOutMergeTests
{
    ...
    [Fact]
    public void Empty_fetch_marks_partition_exhausted_so_next_page_skips_it()
    {
        var t0 = DateTime.UtcNow;
        var fetched = new[]
        {
        ...
        Assert.True(conts["Agent"].Exhausted);
        Assert.False(conts["Security"].Exhausted);
    }
    ...
    public void Continuation_keeps_position_for_partitions_whose_items_lost_the_merge_cut()
    {
        ...
        // (null = restart). The partition is NOT marked exhausted, so the next page
        // will re-fetch its items and they'll be re-considered at a lower floor.
        Assert.False(conts["Agent"].Exhausted);
    ...
    [Fact]
    public void Encode_then_decode_round_trips_continuation_map()
    {
        var original = new Dictionary<string, PartitionContinuation>
        {
            ["Agent"]    = new PartitionContinuation("0123456789012345678_aaa", false),
            ["Security"] = new PartitionContinuation(null, true),
            ["Consent"]  = new PartitionContinuation("0987654321098765432_bbb", false),
        };
        ...
        Assert.False(decoded["Agent"].Exhausted);
        Assert.True(decoded["Security"].Exhausted);
    ...
    public void Multi_page_drain_returns_all_items_globally_newest_first_no_duplicates()
    {
        ...
        IReadOnlyList<PartitionFetchResult<OpsEventEntry>> SimulateFetch(
            Dictionary<string, PartitionContinuation> continuations, int pageSize)
        {
            var fetched = new List<PartitionFetchResult<OpsEventEntry>>();
            foreach (var part in partitions)
            {
                if (continuations.TryGetValue(part, out var c) && c.Exhausted) continue;
        ...
        while (true)
        {
            var fetched = SimulateFetch(conts, pageSize);
            var (page, nextConts) = MergeAndAdvance(fetched, conts, pageSize, e => e.Timestamp);
            seen.AddRange(page);
            conts = nextConts;

            bool anyActive = conts.Any(kv => !kv.Value.Exhausted);
            if (!anyActive) break;
