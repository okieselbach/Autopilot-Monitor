 ...
 * triggered from the In-Flight / Poisoned / Stranded tabs) and inline in the Restore Browser
 * tab's right-hand preview panel. Fetches `mode=summary` on its own and renders the same
 * counts + progress + last-failure breakdown either way.
 */
export interface DeletionManifestSummaryViewProps {
  tenantId: string;
  sessionId: string;
  manifestId: string;
  getAccessToken: (forceRefresh?: boolean) => Promise<string | null>;
  /** Renders an extra "Download snapshot" button below the summary when true. */
...
/** Matches CascadeVerificationService.MaxResidualSampleSize on the backend. */
const VERIFIER_OBSERVATION_CAP = 50;

export function DeletionManifestSummaryView({
  tenantId,
  sessionId,
  manifestId,
  getAccessToken,
  showDownload = true,
}: DeletionManifestSummaryViewProps) {
  ...
  if (loading) {
    return <p className="text-sm text-gray-600 dark:text-gray-400">Loading stored manifest…</p>;
  }
  if (error) {
    return (
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 text-sm">
        ...
        <Field label="Snapshot SHA-256" value={summary.snapshotSha256 ?? "—"} mono />
        <Field
          label="Snapshot size"
          value={summary.estimatedSnapshotBytes != null ? formatBytes(summary.estimatedSnapshotBytes) : "—"}
        />
      </div>
        ...
        <div className="border border-purple-200 dark:border-purple-700 bg-purple-50 dark:bg-purple-900/20 rounded p-3 text-sm">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <span className="text-gray-700 dark:text-gray-300">
              Completed steps: <strong>{completedCount}</strong>
            </span>
            <span className="text-gray-700 dark:text-gray-300">
              Verification done: <strong>{progress.verificationDone ? "yes" : "no"}</strong>
            </span>
            <span className="text-gray-700 dark:text-gray-300">
              Tombstone started: <strong>{progress.tombstoneStarted ? "yes" : "no"}</strong>
            </span>
            <span className="text-gray-700 dark:text-gray-300">
              Aggregate decrements: <strong>{progress.aggregateDecrementsApplied ?? 0}</strong>
            </span>
          </div>
          {progress.lastFailureType && (
            <div className="mt-3 p-2 bg-red-100 dark:bg-red-900/40 rounded text-xs text-red-800 dark:text-red-100">
              <div>
                <strong>Last failure:</strong> <code>{progress.lastFailureType}</code>
              </div>
              {progress.lastFailureMessage && <div className="mt-1 break-words">{progress.lastFailureMessage}</div>}
                ...
                return (
                  <details className="mt-2">
                    <summary className="cursor-pointer text-red-700 dark:text-red-200">
                      Residual sample ({sampleSize} of {observed}
                      {capHit ? "+" : ""} observed row{observed === 1 ? "" : "s"})
                    </summary>
                    ...
                    {capHit && (
                      <p className="mt-2 text-xs italic text-red-700 dark:text-red-200">
                        The verifier short-circuits at {VERIFIER_OBSERVATION_CAP} rows per table and after the first
                        failing table — the real residual count may be higher. Use the cascade-restore path to recover;
                        do not rely on this number as an exact blast-radius estimate.
                      </p>
                    )}
                  </details>
                );
          ...
          <div className="border border-gray-200 dark:border-gray-700 rounded overflow-hidden">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                {Object.entries(summary.preflightCounts)
                  .sort(([, a], [, b]) => b - a)
                  .map(([table, count]) => (
                    <tr key={table}>
