export function SectionSetup() {
  return (
    <section className="bg-white rounded-lg shadow-md p-8">
      ...
      <div className="mt-8 p-5 bg-gradient-to-r from-indigo-50 to-blue-50 border border-indigo-100 rounded-lg">
        <div className="flex items-center space-x-2 mb-3">
          ...
          <p className="font-semibold text-indigo-900">Recommended Configuration</p>
        </div>
        ...
        <ul className="space-y-3 text-sm text-gray-700">
          <li className="flex gap-2">
            ...
            <div>
              <strong>Agent Settings</strong> — Enable{" "}
              <span className="font-mono text-xs bg-white/70 px-1.5 py-0.5 rounded">Geo Location Detection</span> and{" "}
              <span className="font-mono text-xs bg-white/70 px-1.5 py-0.5 rounded">Set Timezone Automatically</span>{" "}
              so that the device timezone is set correctly during enrollment — without a location sensor, Windows
              defaults to Pacific Standard Time (PST).
            </div>
          </li>
          <li className="flex gap-2">
            <span className="text-indigo-500 font-bold mt-0.5">2.</span>
            <div>
              <strong>Agent Analyzers</strong> — Enable the{" "}
              <span className="font-mono text-xs bg-white/70 px-1.5 py-0.5 rounded">Local Admin Analyzer</span> (configure
              your allowed admin accounts to detect unauthorized local admins),{" "}
              <span className="font-mono text-xs bg-white/70 px-1.5 py-0.5 rounded">Software Inventory</span>, and the{" "}
              <span className="font-mono text-xs bg-white/70 px-1.5 py-0.5 rounded">Vulnerability Analyzer</span> for
              security insights during enrollment.
            </div>
          </li>
          <li className="flex gap-2">
            <span className="text-indigo-500 font-bold mt-0.5">3.</span>
            <div>
              <strong>Diagnostics Package</strong> — Enable diagnostics collection to automatically gather log files
              from devices for troubleshooting. This requires your own Azure Blob Storage account to store the
              uploaded packages.
            </div>
          </li>
        </ul>
