...
describe("api.sessions.stats — per-tenant", () => {
  it("uses /api/sessions/stats with no params by default", async () => {
    const { api } = await apiPromise;
    expect(api.sessions.stats()).toBe("https://test.example/api/sessions/stats");
  });

  it("honours an explicit days override", async () => {
    const { api } = await apiPromise;
    expect(api.sessions.stats({ days: 30 })).toBe(
      "https://test.example/api/sessions/stats?days=30",
    );
  });
  ...
  it("omits days when undefined", async () => {
    ...
    expect(api.sessions.stats({ days: undefined })).toBe(
      "https://test.example/api/sessions/stats",
    );
  });
});

describe("api.globalSessions.stats — GA cross-tenant", () => {
  it("uses /api/global/sessions/stats with no params by default", async () => {
    const { api } = await apiPromise;
    expect(api.globalSessions.stats()).toBe(
      "https://test.example/api/global/sessions/stats",
    );
  });
  ...
  it("forwards an optional tenantId filter", async () => {
    ...
    expect(
      ...
      "https://test.example/api/global/sessions/stats?tenantId=11111111-1111-1111-1111-111111111111",
    );
  });
  ...
  it("forwards both tenantId and days together", async () => {
    ...
    expect(
      ...
      "https://test.example/api/global/sessions/stats?tenantId=22222222-2222-2222-2222-222222222222&days=14",
    );
  });
  ...
  it("omits an empty-string tenantId so the backend treats it as platform-wide", async () => {
    ...
    expect(api.globalSessions.stats({ tenantId: "", days: 7 })).toBe(
      "https://test.example/api/global/sessions/stats?days=7",
    );
  });
});
